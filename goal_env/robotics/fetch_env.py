import numpy as np
from goal_env.robotics import rotations, robot_env, utils
import copy
...
class FetchEnv(robot_env.RobotEnv):
    ...
    def _get_obs(self):
        ...
        obs = np.concatenate([
            object_pos.ravel(), grip_pos, object_rel_pos.ravel(), gripper_state, object_rot.ravel(),
    ...
    def _sample_goal(self):
        if self.has_object:
            if type(self.target_range) is float:
                goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-self.target_range, self.target_range,
                                                                              size=3)
            else:
                goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(self.target_range[0],
                                                                              self.target_range[1])
